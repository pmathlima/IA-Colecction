...
import { ProductCategory, ProductFilters } from '../../../core/models/product.model';
...
export class ProductFiltersComponent {
  ...
  updateTerm(termo: string): void {
    this.emit({ ...this.filters(), termo });
  }

  updateCategory(categoria: ProductCategory | 'Todas'): void {
    this.emit({ ...this.filters(), categoria });
  }

  updatePrice(precoMaximo: number): void {
    this.emit({ ...this.filters(), precoMaximo });
  }

  resetFilters(): void {
    this.emit({ termo: '', categoria: 'Todas', precoMaximo: this.highestPrice() });
  }
